...
namespace dsf::mobility {
  ...
  enum class AgentInsertionMethod : uint8_t {
    RANDOM = 0,  // Agents spwan randomly in the network and travel with no destination
  ...
  template <typename delay_t>
    requires(is_numeric_v<delay_t>)
  void RoadDynamics<delay_t>::m_addAgentsRandom(std::size_t nAgents) {
  ...
  template <typename delay_t>
    requires(is_numeric_v<delay_t>)
  void RoadDynamics<delay_t>::m_addAgentsODs(std::size_t nAgents) {
    if (m_ODs.empty()) {
  ...
  template <typename delay_t>
    requires(is_numeric_v<delay_t>)
  void RoadDynamics<delay_t>::m_addAgentsRandomODs(std::size_t nAgents) {
    m_nAddedAgents += nAgents;
  ...
  template <typename delay_t>
    requires(is_numeric_v<delay_t>)
  void RoadDynamics<delay_t>::addAgents(std::size_t const nAgents,

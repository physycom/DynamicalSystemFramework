...
int main(int argc, char* argv[]) {
  ...
  tl1.setCycle(s01.id(), dsf::Direction::ANY, {static_cast<dsf::Delay>(std::ceil(62 * TIME_SCALING)), 0});
  // Via Donato Creti
  graph.addNode<TrafficLight>(2, 141);
  auto& tl2 = graph.node<TrafficLight>(2);
  tl2.setCycle(s12.id(), dsf::Direction::ANY, {static_cast<dsf::Delay>(std::ceil(72 * TIME_SCALING)), 0});
  // Via del Lavoro
  graph.addNode<TrafficLight>(3, 138);
  auto& tl3 = graph.node<TrafficLight>(3);
  tl3.setCycle(s23.id(), dsf::Direction::ANY, {static_cast<dsf::Delay>(std::ceil(88 * TIME_SCALING)), 0});
  // Viali
  graph.addNode<TrafficLight>(4, 131);
  auto& tl4 = graph.node<TrafficLight>(4);
  tl4.setCycle(s34.id(), dsf::Direction::ANY, {static_cast<dsf::Delay>(std::ceil(81 * TIME_SCALING)), 0});
  ...
  std::ofstream ofs{std::format("./stalingrado_output_tl_{:.1f}_{:.2f}.csv", SCALING_FACTOR, TIME_SCALING)};
  std::ofstream ofs_queues{std::format("./stalingrado_tl_queues_{:.1f}_{:.2f}.csv", SCALING_FACTOR, TIME_SCALING)};
  ...
  while (progress < MAX_TIME) {
    if (progress % 60 == 0) {
      ...
      dynamics.addAgents(static_cast<std::size_t>(std::round(*it * SCALING_FACTOR)), 4, 0);
    }
    if (progress % 30 == 0) {

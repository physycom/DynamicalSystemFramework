...
TEST_CASE("FirstOrderDynamics") {
  ...
  SUBCASE("TrafficLights") {
    ...
    GIVEN(
      ...
      tl.setCycle(1, dsm::Direction::ANY, {2, 0});
      tl.setCycle(16, dsm::Direction::ANY, {2, 2});
      graph2.buildAdj();
      FirstOrderDynamics dynamics{
          graph2, false, 69, 0., dsm::weight_functions::streetLength, 1.};
      dynamics.addItinerary(2, 2);
      ...
      GIVEN("A dynamics object with a traffic light intersection") {
        ...
        WHEN("We evolve the dynamics and optimize traffic lights") {
          ...
          THEN("Green and red time are different") {
            CHECK(tl.cycles().at(1).at(dsm::Direction::ANY).greenTime() >
                  tl.cycles().at(16).at(dsm::Direction::ANY).greenTime());
            CHECK_EQ(tl.cycles().at(1).at(dsm::Direction::ANY).greenTime(),
                     tl.cycles().at(11).at(dsm::Direction::ANY).greenTime());
            CHECK_EQ(tl.cycles().at(16).at(dsm::Direction::ANY).greenTime(),
                     tl.cycles().at(21).at(dsm::Direction::ANY).greenTime());
          }
        }
        WHEN(
